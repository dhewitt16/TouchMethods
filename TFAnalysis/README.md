# Time-frequency (ERD/S) Analysis

Here we demonstrate a method to analyse cortical oscillations using the event-related desynchronisation method following the offset of dynamic touch. The following preprocessing steps can be followed. This method was used in Hewitt et al. (2025), SCAN, doi: 10.1093/scan/nsaf082.  
...
## Epoching the data
4.	Epoch into trials and concatenate conditions into one file – using EpochFilesSA.m.  
...
## Data Cleaning
5.	Data cleaning – we use a semi-automatic pipeline here, though different methods including manual rejection can also be used. 
...
The next section runs data cleaning in parallel for time-frequency and ERP data. Different methods (such as marking artifacts rather than rejecting epochs) can also be used and may be more time efficient, depending on the study and the volume of data.  
...
d.	Relative power in the band calculated by specifying the baseline interval (2 to -1), and computing relative change versus the trial – baseline correction not carried out at this stage.  
...
## Exporting for statistics and visualisation
